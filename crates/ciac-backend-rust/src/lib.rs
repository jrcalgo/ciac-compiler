...
/// (v0.22 M1 — `22UpdatePlan.md` Pillar 1).
///
/// `validate` was two steps until it was measured: `cargo check` with
/// `RUSTFLAGS=-D warnings`, then `cargo test --lib --tests` without
/// them. Because the two disagree on `RUSTFLAGS` they share no
/// fingerprint, and `check` only ever emits metadata `test` cannot
/// link against — so the second step rebuilt from scratch everything
/// the first had just built. Measured on a generated `ping` project:
/// 16.5s + 20.2s = 36.8s, against 19.8s for the single step below,
/// which denies warnings *and* runs the tests. One step, strictly more
/// proof, ~45% less wall time. `ci_test_steps` above still emits the
/// two-step form into generated workflows — collapsing that too would
/// change generated bytes, which this does not.
static TARGET_INFO: TargetInfo = TargetInfo {
    ...
    validate: &[ValidateStep {
        ...
        purpose: "type-checks (deny warnings), unit and generated tests pass",
    }],
