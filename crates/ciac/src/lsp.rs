...
const CAPABILITIES: &[(&str, &str)] = &[
    ...
    ("db", "Capability: relational persistence. Providers: `Postgres` (both targets), `MySQL` (Python only — the Rust target reports CIAC0011)."),
    ("cache", "Capability: key-value cache. Provider: `Redis` (both targets)."),
    ("queue", "Capability: message broker backing `stream`/`worker`. Providers: `NATS` (both targets), `Kafka` (Python only — the Rust target reports CIAC0011)."),
...
const PROVIDERS: &[(&str, &str)] = &[
    ...
    ("MySQL", "db provider (Python only — the Rust target reports CIAC0011)."),
    ("Redis", "cache provider (both targets)."),
    ("NATS", "queue provider (both targets)."),
    ("Kafka", "queue provider (Python only — the Rust target reports CIAC0011)."),
