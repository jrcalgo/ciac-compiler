...
| --- | --- | --- |
| Simulation depth: only `db.insert` + publish faked | Java | `27UpdatePlan.md` M8 — Rust CLOSED at M4, TypeScript CLOSED at M6, Go CLOSED at M7 (gate-emptiness proven across the whole example corpus, live-verified against nine corpus scenarios each; proof recorded in each milestone's own Shipped note) |
